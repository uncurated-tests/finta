import { PageObjectResponse, PropertyItemListResponse } from "@notionhq/client/build/src/api-endpoints";
import { TableConfigFields } from "../../types/shared";

const getValueFromProperty = (property: PropertyItemListResponse['results'][0]) => {
...
export const parsePageProperties = ({ page, tableConfigFields }: { page: PageObjectResponse, tableConfigFields: Record<TableConfigFields, string> | {} }) => {
  ...
  return Object.fromEntries(Object.values(page.properties)
    .map(property => {
      ...
      const value = getValueFromProperty(property as any as PropertyItemListResponse['results'][0]);
      return [ field, value ]
